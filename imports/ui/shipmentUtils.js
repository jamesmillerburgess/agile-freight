...
const originCountry = shipment => {
  ...
  if (shipment.movement.receipt) {
    return Countries.findOne({
      countryCode: shipment.movement.receipt.countryCode,
    }).name;
  }
  if (shipment.movement.departure) {
    return Countries.findOne({
      countryCode: shipment.movement.departure.countryCode,
    }).name;
  }
};
...
const destinationCountry = shipment => {
  ...
  if (shipment.movement.delivery) {
    return Countries.findOne({
      countryCode: shipment.movement.delivery.countryCode,
    }).name;
  }
  if (shipment.movement.arrival) {
    return Countries.findOne({
      countryCode: shipment.movement.arrival.countryCode,
    }).name;
  }
  return '';
};
...
const cargoString = shipment => {
  ...
  if (cargo.cargoType === 'Containerized') {
    ...
    return `${cargo.totalContainers} UNIT${cargo.totalContainers !== 1
      ? 'S'
      : ''}, ${cargo.totalTEU} TEU`;
...
const filter = (shipment, options) => {
  const isActive = !isInactive(shipment);
  if (options.showActive === false && isActive) {
    return false;
  }
  if (options.showInactive === false && !isActive) {
    return false;
  }
  let shipmentMode = 'n/a';
  if (shipment.movement && shipment.movement.mode) {
    shipmentMode = shipment.movement.mode;
  }
  if (options.showAir === false && shipmentMode === 'Air') {
    return false;
  }
  if (options.showSea === false && shipmentMode === 'Sea') {
    return false;
  }
  if (options.showRoad === false && shipmentMode === 'Road') {
    return false;
  }
  if (options.showBrokerage === false && shipmentMode === 'Brokerage') {
    return false;
  }
  return true;
};
